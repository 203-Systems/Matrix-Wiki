...
## Saved Variables
The NVS API is rather complex requiring the use of a hash to identify variables. In most case you will want to use the SavedVar to create saved variables instead of using the NVS API directly.

You will want to use the NVS API directly if you need to store a large amount of data like save the full configuration for Note layout after user is done configurating instead saving after every small changes with large numbers of saved variables.
...
**Returns:**
- `size_t`: The size of the variable in bytes. If the variable does not exist, the returned size may indicate an invalid or empty variable.
...
**Returns:**
- `vector<char>`: The content of the variable. If the variable does not exist, the returned vector may be empty.
