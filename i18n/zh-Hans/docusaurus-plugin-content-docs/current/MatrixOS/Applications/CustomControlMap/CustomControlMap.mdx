...
自定义控制应用是一个强大的工具，允许你为 Matrix OS 设备创建自定义控制界面。你可以使用 [Matrix OS 自定义控制编辑器](https://edit.203.io) 来创建自定义控制界面。
...
当你在自定义控制界面模式下长按功能键时，你将进入操作菜单，该菜单提供对各种设置和控制的访问。
...
export function ActionMenu() {
    const uiElements = [
        ...
        {
            "name": "重新加载",
            "desc": "重新加载当前控制界面配置。（这样图层配置将重置界面的默认值）",
            "elements": [
                {
                    "pos": [0, 5],
                    "color": "#FF0000"
    ...
    const uiDescription = "操作菜单提供对控制界面设置、图层管理和系统功能的访问。通过长按功能键访问。"
    const uiParentLink = "#"
